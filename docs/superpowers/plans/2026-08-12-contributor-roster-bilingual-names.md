...
- Preserve exactly 12 confirmed GitHub mappings; never infer a GitHub account from email, avatar similarity, or a similar username.
...
Yipeng Lin | 林依鹏 | YL | 林 | https://github.com/doublemoon-bird ; team-introduction-2026-06-30
...
For every row, set `role` to `member` except Xiangfeng Wang, whose role is `advisor`. Preserve the 12 GitHub values in `EXPECTED_GITHUB`; use JSON `null` for all other GitHub fields. Treat each semicolon-separated source above as a separate string in the record's `evidence` array.
...
Expected: all tests PASS with 20 cards, 12 linked cards, 8 static cards, Conan Xu and Dong Yuan first, and the advisor last in both languages.
...
Expected: PASS; JSON reports 20 cards, 12 linked, 8 static, no horizontal overflow at 1366×900 or 390×844, correct name order in both languages, and four screenshot paths under `$VM_SCREENSHOT_DIR`.
